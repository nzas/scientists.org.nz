<?php
// $Id: watcher.module,v 1.2.2.6.2.25 2009/07/12 10:17:24 solipsist Exp $
/**
 * @file
 * Watcher module.
 *
 ...
 * @param $dest
 *    A destination path, used for redirection afterwards, should be current path
 *
 * @return
 *    A themed icon for toggling email notifications
 */
function theme_watcher_binder_email_icon($t, $type, $url, $dest = null) {
  $output = l(
 ...
 * @param $dest
 *    A destination path, used for redirection, should be current path
 *
 * @return
 *    A themed link for stopping watching a node
 */
function theme_watcher_binder_stop_watching_icon($url, $dest = null) {
  $output = l(
  t('Stop watching'),
  $url,
  array(
 ...
 *
 */
function theme_watcher_node_toggle_watching_link($uid, $nid, $dest, $user_is_watching, $t = array()) {
...
function watcher_theme($existing, $type, $theme, $path) {
  return array(
    ...
    'watcher_binder_email_icon' => array(
      'arguments' => array('t' => null, 'type' => null, 'url' => null, 'dest' => null),
  ),
    'watcher_binder_stop_watching_icon' => array(
      'arguments' => array('url' => null, 'dest' => null),
  ),
...
function _watcher_binder($account) {
  ...
  while ($node = db_fetch_object($result)) {
    ...
    if ($user_is_owner && $emails_enabled) {
      $row[] = theme('watcher_binder_email_icon', $tstrings, ($node->send_email ? 'enabled' : 'disabled'), "user/$uid/watcher/email_notifications_toggle/$node->nid", $dest);
    }

    // Stop watching icon
    if ($user_is_owner) {
      $row[] = theme('watcher_binder_stop_watching_icon', "user/$uid/watcher/toggle/$node->nid", $dest);
    }
  ...
  if ($mail && $unwatch) {
    ...
    if ($unwatch == 'node') {
      ...
      if (_watcher_user_unset_watching_nodes(array($nid), $recipient_user)) {
        $message = t('%ntitle previously watched by %email is no longer being watched.', array('%ntitle' => $node->title, '%email' => $recipient_user->mail));
    ...
    if ($unwatch == 'all') {
      if ($confirm != 'confirm') {
        ...
        if ($db_result = _watcher_db_get_watched_nodes_mini($recipient_user)) {
          ...
          $message = t('All posts on this site previously watched by %email are no longer being watched.', array('%email' => $recipient_user->mail));
        }
      }
    }
  }
  ...
  // Handle watching and unwatching for logged-in users
    ...
    if (_watcher_user_is_watching_node($user, $nid)) {
      if (_watcher_user_unset_watching_nodes(array($nid), $user)) {
        $message = t('You are no longer watching <em>!ntitle</em>. <strong><a href="!undo">Undo</a></strong>', array('!ntitle' => $node->title, '!undo' => url("user/$user->uid/watcher/toggle/$node->nid", array('query' => $dest))));
      ...
      if (_watcher_user_set_watching_node($nid, $user)) {
        $message = t('You are now watching <em>!ntitle</em>. <strong><a href="!undo">Undo</a></strong>', array('!ntitle' => $node->title, '!undo' => url("user/$user->uid/watcher/toggle/$node->nid", array('query' => $dest))));
...
function _watcher_email_notify_node_update(&$node) {
  ...
  // Set up subject
  $node_title = ( $node->title ? ' "'. substr($node->title, 0, 128) .'" ' : false );
...
function _watcher_email_notify_anonymous_watch_confirm($user, $nid, $what = null) {
  ...
  // Set up subject
  $node_title = ( $node->title ? t(' "!nodetitle"', array('!nodetitle' => substr($node->title, 0, 128))) : false );
  $site_name = ( variable_get('site_name', false ) ? ' ('. variable_get('site_name', '') .')' : '' );
...
function _watcher_node_watch_link(&$node, $a3, $a4) {
  ...
  $tstrings = array();
  $tstrings['watch_toggle_enabled'] = t('Click to stop watching');
  ...
  $tstrings['watch_toggle_disabled'] = t('Click to start watching');
...
function _watcher_watch_toggle_anonymous_form(&$form_state, $nid, $watch_action){
  ...
  if ($watch_action == 'unwatch') {
    $form['watcher'] = array (
      '#type' => 'fieldset',
      '#title' => '<strong>'. t("Stop watching %title", array('%title' => $node->title)) .'</strong>',
    );
    $info = t("Enter your email address below to stop watching %title. The form will remember the email you enter until next time.", array('%title' => $node->title, '%ipinfo' => ( $ip_reported ? $ip_info : null )));
  }
  else {
    $form['watcher'] = array (
      '#type' => 'fieldset',
      '#title' => '<strong>'. t("Watch %title", array('%title' => $node->title)) .'</strong>',
    );
...
function _watcher_watch_toggle_anonymous_form_submit($form, &$form_state) {
  ...
  if ($watch_action == 'unwatch') {
    if ($user->mail && _watcher_user_unset_watching_nodes(array($nid), $user)) {
      $message = t('You are no longer watching %ntitle. <strong><a href="!undo">Undo</a></strong>', array('%ntitle' => $node->title, '!undo' => url("user/$user->uid/watcher/toggle/$node->nid", array('query' => $dest))));
      drupal_set_message($message);
      ...
      $message = t('That email address is not associated with any user watching %ntitle', array('%ntitle' => $node->title));
      drupal_set_message($message);
    }
  }
  else {
